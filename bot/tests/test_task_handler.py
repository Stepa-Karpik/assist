...
def test_low_risk_task_queues_immediately():
    task_client = FakeTaskClient(task_result=TaskWorkflowResult(status="queued"))
    ...
    assert response == get_queue_success_text()
...
def test_confirm_queues_the_task():
    task_client = FakeTaskClient(
        decision_result=TaskWorkflowResult(status="task_queued")
    )
